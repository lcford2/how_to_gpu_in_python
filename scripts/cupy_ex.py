...
def array_creation(size):
    ...
    e = timer()
    print(f"Array Creation GPUU Time: {e-s:.4} seconds")

    s = timer()
    x_gpu = cp.random.random((size, size, size))
    # sync GPU and GPUU to make sure data gen is done
...
def array_scalar_multiply(x_cpu, x_gpu, scalar):
    ...
    x_cpu = x_cpu * scalar
    e = timer()
    print(f"Array Multiplication GPUU Time: {e-s:.4} seconds")
...
def multiple_operations(x_cpu, x_gpu):
    ...
    mean = x_cpu.mean()
    e = timer()
    print(f"Multiple Ops GPUU Time: {e-s:.4} seconds; Answer={mean:.2f}")
